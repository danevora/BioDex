...
export default function Home() {
  ...
  const handleCapture = async (animalId: string, image: File, confidence?: number) => {
  ...
  const handleCloseDetail = async () => {
    if (newCaptureImageUrl) await refetchCaptures();
        ...
        <section>
          ...
          {filteredAnimals.length > 0 ? (
            ...
            <div className="flex justify-center py-12">
              <p className="text-muted-foreground">No animals match your filters</p>
            </div>
          )}
        </section>
      </main>
